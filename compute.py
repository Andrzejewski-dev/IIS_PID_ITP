...
class UAR:
    ...
    def ComplexOpenData(self):
        ...
        # self.pid['differential_time'] = 1
        # self.pid['integration_time'] = 1
        # self.pid['gain'] = 0.01
        # self.pid['h_z'].append(2)
        # self.pid['A'] = 4
        # self.pid['B'] = 1
        # self.pid['h_max'] = 1000
        # self.pid['u_max'] = 1000
    ...
    def count(self):
        ...
        for n in range(0, self.N):
            ...
            self.pid['h_z'].append(obiekt.pid['h_z'][0])
            self.pid_controler()
            self.valve()
            self.tankk()
        ...
        plt.plot(self.x, self.pid['h'])
        plt.plot(self.x, self.valvee['Q_d'])
        plt.show()

        filePath = 'data_x.json'
        if os.path.exists(filePath):
            ...
            for i in range(0, len(self.x)):
                jsonList.append({"x": float(round(self.x[i],2)), "h_z": self.pid['h_z'][i], "h": self.pid['h'][i],
                                 ...
                                 "e": self.pid['error'][i]})
            with open('data_x.json', 'w') as d_x:
